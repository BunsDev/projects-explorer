...
import { listEvictionCandidates, markCacheEntryEvicted, markJobFailed, markJobSucceeded, reserveJobs, upsertCacheEntry } from "@/lib/cloud/queue-store"
...
function retryDelayMs(attempt: number) {
  return Math.min(60_000, 1_500 * 2 ** Math.max(0, attempt - 1))
}

async function runUpload(task: SyncTask) {
  const body = task.sourceUrl ? await fetchBuffer(task.sourceUrl) : task.localPath ? await import("node:fs/promises").then((m) => m.readFile(task.localPath!)) : null
  ...
  if (existing?.checksumSha256 && existing.checksumSha256 === checksumSha256) {
    await markJobSucceeded(task.id, { bytesTransferred: existing.size, checksumSha256 })
    return
  }

  const stored = await provider.put({ key: task.remoteKey, body, checksumSha256 })
  await markJobSucceeded(task.id, { bytesTransferred: stored.size, checksumSha256 })
...
async function runDownload(task: SyncTask) {
  ...
  const checksumSha256 = result.checksumSha256 || checksum(result.body)
  await markJobSucceeded(task.id, { bytesTransferred: result.body.byteLength, checksumSha256, localPath: targetPath })
  await upsertCacheEntry({
    projectId: task.projectId,
    fileId: task.fileId,
...
async function runEviction(task: SyncTask) {
  ...
  const selected = rankEvictionCandidates(candidates.map((item) => ({ ...item, lastAccessedAt: item.lastAccessedAt ?? new Date(0), sizeBytes: item.sizeBytes ?? 0 })), task.bytesTotal ?? disk.suggestedEvictionBytes)
  for (const item of selected) {
    await rm(item.localPath, { force: true })
  }
  await markCacheEntryEvicted(selected.map((item) => item.id))
  await markJobSucceeded(task.id, { bytesTransferred: selected.reduce((sum, item) => sum + (item.sizeBytes ?? 0), 0) })
}
...
export async function processSyncQueue(options: { concurrency?: number } = {}) {
  ...
  const results = await Promise.allSettled(reserved.map(async (task) => {
    try {
      ...
      await markJobFailed(task.id, message, new Date(Date.now() + retryDelayMs(task.attempts + 1)))
